...
@app.get("/blogs/{blog_id}/comments/{comment_id}", tags=["blogs", "comments"])
def get_comment(
    blog_id: int,
    comment_id: int,
    valid: bool = True,
...
@app.get("/blogs/type/{blog_type}", tags=["blogs"])
def get_blog_type(blog_type: BlogType):
    return {"message": f"Blog type {blog_type}"}


@app.get("/blogs/{blog_id}", status_code=status.HTTP_200_OK, tags=["blogs"])
